...
public class PiglinBruteAi {
    ...
    private static void initFightActivity(PiglinBrute p_35125_, Brain<PiglinBrute> p_35126_) {
        p_35126_.addActivityAndRemoveMemoryWhenStopped(
            ...
            10,
            ImmutableList.of(
                StopAttackingIfTargetInvalid.create((p_375926_, p_375927_) -> !isNearestValidAttackTarget(p_375926_, p_35125_, p_375927_)),
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel p_376673_, AbstractPiglin p_35089_, LivingEntity p_35090_) {
        return findNearestValidAttackTarget(p_376673_, p_35089_).filter(p_35085_ -> p_35085_ == p_35090_).isPresent();
    }

    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel p_376426_, AbstractPiglin p_35087_) {
        Optional<LivingEntity> optional = BehaviorUtils.getLivingEntityFromUUIDMemory(p_35087_, MemoryModuleType.ANGRY_AT);
        if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(p_376426_, p_35087_, optional.get())) {
            return optional;
        } else {
    ...
    protected static void wasHurtBy(ServerLevel p_376396_, PiglinBrute p_35097_, LivingEntity p_35098_) {
        if (!(p_35098_ instanceof AbstractPiglin)) {
            PiglinAi.maybeRetaliate(p_376396_, p_35097_, p_35098_);
        }
    }
