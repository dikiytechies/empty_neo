...
public class LongJump extends Behavior<Breeze> {
    ...
    protected void tick(ServerLevel p_312091_, Breeze p_312923_, long p_312404_) {
        ...
        if (isFinishedInhaling(p_312923_)) {
            Vec3 vec3 = p_312923_.getBrain()
                .getMemory(MemoryModuleType.BREEZE_JUMP_TARGET)
                .flatMap(p_390255_ -> calculateOptimalJumpVector(p_312923_, p_312923_.getRandom(), Vec3.atBottomCenterOf(p_390255_)))
                .orElse(null);
            if (vec3 == null) {
    ...
    private static boolean canJumpFromCurrentPosition(ServerLevel p_312261_, Breeze p_312824_) {
        ...
        if (p_312261_.getBlockState(blockpos).is(Blocks.HONEY_BLOCK)) {
            return false;
        } else {
            for (int i = 1; i <= 4; i++) {
    ...
    private static Optional<Vec3> calculateOptimalJumpVector(Breeze p_312651_, RandomSource p_312364_, Vec3 p_312758_) {
        for (int i : Util.shuffledCopy(ALLOWED_ANGLES, p_312364_)) {
            float f = 0.058333334F * (float)p_312651_.getAttributeValue(Attributes.FOLLOW_RANGE);
            Optional<Vec3> optional = LongJumpUtil.calculateJumpVectorForAngle(p_312651_, p_312758_, f, i, false);
            if (optional.isPresent()) {
                if (p_312651_.hasEffect(MobEffects.JUMP)) {
                    double d0 = optional.get().normalize().y * (double)p_312651_.getJumpBoostPower();
