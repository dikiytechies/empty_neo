...
public class TrialSpawnerData {
        ...
        if (!flag) {
            if (!p_338452_.getState().equals(TrialSpawnerState.COOLDOWN) || !p_338452_.isOminous()) {
                ...
                if (!p_338452_.isOminous() && !list.isEmpty()) {
                    ...
                    optional.ifPresent(p_376073_ -> {
                        Player player = p_376073_.getFirst();
                        if (p_376073_.getSecond() == MobEffects.BAD_OMEN) {
    ...
    public void resetAfterBecomingOminous(TrialSpawner p_338478_, ServerLevel p_338185_) {
        this.currentMobs.stream().map(p_338185_::getEntity).forEach(p_376069_ -> {
            if (p_376069_ != null) {
                p_338185_.levelEvent(3012, p_376069_.blockPosition(), TrialSpawner.FlameParticle.NORMAL.encode());
                if (p_376069_ instanceof Mob mob) {
                    mob.dropPreservedEquipment(p_338185_);
                }

                p_376069_.remove(Entity.RemovalReason.DISCARDED);
            }
        });
        ...
        if (!p_311790_.hasSpinningMob()) {
            ...
            if (this.displayEntity == null) {
                ...
                if (compoundtag.contains("id", 8)) {
                    this.displayEntity = EntityType.loadEntityRecursive(compoundtag, p_312148_, EntitySpawnReason.TRIAL_SPAWNER, Function.identity());
                }
            }
