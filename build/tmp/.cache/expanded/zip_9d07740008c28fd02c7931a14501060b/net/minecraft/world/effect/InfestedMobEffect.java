...
class InfestedMobEffect extends MobEffect {
    ...
    public void onMobHurt(ServerLevel p_376338_, LivingEntity p_338481_, int p_338438_, DamageSource p_338894_, float p_338367_) {
        if (p_338481_.getRandom().nextFloat() <= this.chanceToSpawn) {
            ...
            for (int j = 0; j < i; j++) {
                this.spawnSilverfish(p_376338_, p_338481_, p_338481_.getX(), p_338481_.getY() + (double)p_338481_.getBbHeight() / 2.0, p_338481_.getZ());
            }
        }
    }

    private void spawnSilverfish(ServerLevel p_376716_, LivingEntity p_341023_, double p_338557_, double p_338848_, double p_338562_) {
        Silverfish silverfish = EntityType.SILVERFISH.create(p_376716_, EntitySpawnReason.TRIGGERED);
        if (silverfish != null) {
            ...
            silverfish.moveTo(p_338557_, p_338848_, p_338562_, p_376716_.getRandom().nextFloat() * 360.0F, 0.0F);
            silverfish.setDeltaMovement(new Vec3(vector3f));
            p_376716_.addFreshEntity(silverfish);
