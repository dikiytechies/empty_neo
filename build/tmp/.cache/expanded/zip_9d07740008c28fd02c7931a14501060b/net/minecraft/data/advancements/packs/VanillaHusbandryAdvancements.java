...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    public static AdvancementHolder createBreedAllAnimalsAdvancement(
        AdvancementHolder p_300959_,
        Consumer<AdvancementHolder> p_266923_,
        HolderGetter<EntityType<?>> p_361750_,
        Stream<EntityType<?>> p_266961_,
        Stream<EntityType<?>> p_266751_
    ) {
        return addBreedable(Advancement.Builder.advancement(), p_266961_, p_361750_, p_266751_)
            .parent(p_300959_)
            .display(
                Items.GOLDEN_CARROT,
    ...
    private static Advancement.Builder addLeashedFrogVariants(
        HolderGetter<EntityType<?>> p_360794_, HolderGetter<Item> p_360803_, Advancement.Builder p_249739_
    ) {
        BuiltInRegistries.FROG_VARIANT
            .listElements()
            .forEach(
                p_359387_ -> p_249739_.addCriterion(
                        p_359387_.key().location().toString(),
                        PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                            ItemPredicate.Builder.item().of(p_360803_, Items.LEAD),
                            Optional.of(
                                EntityPredicate.wrap(
                                    EntityPredicate.Builder.entity().of(p_360794_, EntityType.FROG).subPredicate(EntitySubPredicates.frogVariant(p_359387_))
                                )
                            )
                        )
                    )
    ...
    private static Advancement.Builder addFood(Advancement.Builder p_248532_, HolderGetter<Item> p_362606_) {
        for (Item item : EDIBLE_ITEMS) {
            p_248532_.addCriterion(BuiltInRegistries.ITEM.getKey(item).getPath(), ConsumeItemTrigger.TriggerInstance.usedItem(p_362606_, item));
        }
    ...
    private static Advancement.Builder addBreedable(
        Advancement.Builder p_266978_, Stream<EntityType<?>> p_267147_, HolderGetter<EntityType<?>> p_364934_, Stream<EntityType<?>> p_267091_
    ) {
        p_267147_.forEach(
            p_359390_ -> p_266978_.addCriterion(
                    EntityType.getKey((EntityType<?>)p_359390_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(p_364934_, (EntityType<?>)p_359390_))
                )
        );
        p_267091_.forEach(
            p_359383_ -> p_266978_.addCriterion(
                    EntityType.getKey((EntityType<?>)p_359383_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(
                        Optional.of(EntityPredicate.Builder.entity().of(p_364934_, (EntityType<?>)p_359383_).build()),
                        Optional.of(EntityPredicate.Builder.entity().of(p_364934_, (EntityType<?>)p_359383_).build()),
                        Optional.empty()
                    )
                )
    ...
    private static Advancement.Builder addFishBuckets(Advancement.Builder p_249285_, HolderGetter<Item> p_364364_) {
        for (Item item : FISH_BUCKETS) {
            p_249285_.addCriterion(
                BuiltInRegistries.ITEM.getKey(item).getPath(),
                FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(p_364364_, item))
            );
        }
    ...
    private static Advancement.Builder addFish(Advancement.Builder p_248725_, HolderGetter<Item> p_362829_) {
        for (Item item : FISH) {
            p_248725_.addCriterion(
                BuiltInRegistries.ITEM.getKey(item).getPath(),
                FishingRodHookedTrigger.TriggerInstance.fishedItem(
                    Optional.empty(), Optional.empty(), Optional.of(ItemPredicate.Builder.item().of(p_362829_, item).build())
                )
            );
        }
