...
public record ItemTrimPredicate(Optional<HolderSet<TrimMaterial>> material, Optional<HolderSet<TrimPattern>> pattern)
    ...
    public boolean matches(ItemStack p_341406_, ArmorTrim p_371836_) {
        return this.material.isPresent() && !this.material.get().contains(p_371836_.material())
            ? false
            : !this.pattern.isPresent() || this.pattern.get().contains(p_371836_.pattern());
    }
}
