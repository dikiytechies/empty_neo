...
public class SpawnArmorTrimsCommand {
    ...
    private static int spawnArmorTrims(CommandSourceStack p_266993_, Player p_266983_) {
        ...
        Map<ResourceKey<EquipmentAsset>, List<Item>> map = holderlookup.listElements().map(Holder.Reference::value).filter(p_386298_ -> {
            Equippable equippable1 = p_386298_.components().get(DataComponents.EQUIPPABLE);
            return equippable1 != null && equippable1.slot().getType() == EquipmentSlot.Type.HUMANOID_ARMOR && equippable1.assetId().isPresent();
        }).collect(Collectors.groupingBy(p_386299_ -> p_386299_.components().get(DataComponents.EQUIPPABLE).assetId().get()));
        registry.stream()
            .sorted(Comparator.comparing(p_371348_ -> TRIM_PATTERN_ORDER.applyAsInt(registry.getResourceKey(p_371348_).orElse(null))))
            .forEachOrdered(
                p_371640_ -> registry1.stream()
                        .sorted(Comparator.comparing(p_371790_ -> TRIM_MATERIAL_ORDER.applyAsInt(registry1.getResourceKey(p_371790_).orElse(null))))
                        .forEachOrdered(p_370446_ -> nonnulllist.add(new ArmorTrim(registry1.wrapAsHolder(p_370446_), registry.wrapAsHolder(p_371640_))))
            );
